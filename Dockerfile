FROM debian:latest
EXPOSE 5000
RUN apt-get update && apt-get install -y python3-pip && apt-get install sudo -y&& apt-get install git -y && apt-get install systemd -y && apt-get install libpcre3 -y && apt-get clean && rm -rf /var/lib/apt/lists/*
WORKDIR /home/Python
